...
- MODIFICACIÓ: Data. Autor. Descripció.
*/
void CBlobResult::AddBlob( BlobPtr blob )
{
...
BlobPtr CBlobResult::GetBlobPtr(int indexblob)
{	
	if( indexblob < 0 || indexblob >= GetNumBlobs() )
		RaiseError( EXCEPTION_BLOB_OUT_OF_BOUNDS );
...
- PARAMETERS:
	- name: property name (CBlobOperador name)
- RESULT:
- RESTRICTIONS:
- AUTHOR: Ricard Borràs
...
- MODIFICATION: Date. Author. Description.
*/
void CBlobResult::RemoveProperty( const std::string &name )
{
