										 ...
										 pAboveMask++, pBelowMask++ )
		{
			...
			if( externalContour )
			{
				...
				*pLabels = currentLabel;
				...
				// create new blob
				currentBlob = BlobPtr( new Blob(currentLabel, currentPoint, imageSizes ) );

				// contour tracing with currentLabel
				contourTracing( inputImage, maskImage, currentPoint, 
                        labelledImage, visitedPoints, 
                        currentLabel, false, backgroundColor,
                        currentBlob->externalContour() );
			...
			else 
			{
				if( internalContour )
				{
					...
					if(contourLabel>0)
					{
						...
						BlobContour newContour(currentPoint, currentBlob->storage());
...
void contourTracing( IplImage *image, 
					 ...
					 bool internalContour, unsigned char backgroundColor, BlobContour *currentBlobcontour )
{
